...
| Intranet and standalone systems | Scanning, duplicate proof, planning, verification, and default narration all work locally. Optional `--ollama-narrative` sends opaque metadata only to a fixed local loopback endpoint; cloud narration is separately opted in and neither can control a decision. | The selected local-model runtime/provisioning and any movement of a local brief remain operator policy. SANCHAY makes no direct remote request for the Ollama path. |
