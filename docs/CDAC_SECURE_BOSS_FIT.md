...
| --- | --- | --- |
| Critical end nodes and hardening | The core has no autonomous cleanup, daemon, root-command runner, or network dependency. Interactive file actions are disabled by default and require temporary permission, exact confirmation, and evidence revalidation. | Does not make SANCHAY a security-control replacement. |
| ISOC/client monitoring | `--operator-brief` generates a checksum-bearing aggregate handoff with no paths, names, process identifiers, mount sources, or file content. It is write-once unless an operator explicitly requests replacement. Schema-6 snapshot histories likewise carry a local SHA-256 checksum and `--verify-snapshot` is read-only. When a capacity-risk horizon was requested, the brief can include only the aggregate risk probability, horizon, sample evidence, and model metrics. The checksums reveal a mismatch against stored content; they are not signatures. | It does not transmit the brief, call an ISOC API, sign an event, attest a device, or claim SIEM integration. |
| Intranet and standalone systems | Scanning, duplicate proof, local classification, planning, verification, and default narration work locally. Optional `--ai-provider ollama` sends bounded path-free records only to fixed loopback; its structured result may confirm a permitted review or change it to Keep. API reasoning is separately configured and explicit. | The selected model/runtime and any movement of a local brief remain operator policy. Neither reasoning route can add a protected candidate, bypass a deterministic gate, or execute a file action. |
