...
| ISOC/client monitoring | `--operator-brief` generates a checksum-bearing aggregate handoff with no paths, names, process identifiers, mount sources, or file content. Its SHA-256 value detects accidental brief changes; it is not a signature. | It does not transmit the brief, call an ISOC API, sign an event, or claim SIEM integration. |
