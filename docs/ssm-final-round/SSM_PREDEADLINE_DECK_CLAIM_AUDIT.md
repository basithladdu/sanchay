...
| --- | --- | --- | --- |
| “100% Zero Loss” / “guarantee” | SANCHAY cannot guarantee a later human's independent action, an operator's retention policy, storage durability, or absence of every defect. | “SANCHAY has no automatic executor; its guarded `/delete`, `/move`, and `/clean` commands require explicit user intent, while unique and hardlinked entries are excluded from cleanup.” | `python3 -m sanchay.demo --prove` prints that no file was deleted, moved, or transmitted. |
...
| “The LLM sees only 100% regenerable files” | The constrained reasoner receives prefiltered cleanup and archive reviews, so “100% regenerable” is unjustified. | “The optional reasoner receives path-free records and candidate-specific allowed actions. It may confirm a permitted review or change it to Keep; it cannot promote a protected file, bypass a gate, or execute cleanup.” | `sanchay/advisor.py`; advisor-boundary tests. |
